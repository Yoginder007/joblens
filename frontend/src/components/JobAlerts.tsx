...
export default function JobAlerts({ resumeId, filters }: JobAlertsProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 relative">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={handleCreate} disabled={busy}
            className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-bold transition-all
      ...
      {error && <p className="text-xs text-destructive mt-3 relative">{error}</p>}
            ...
            {activeAlerts.map((a) => (
              <motion.div key={a.id}
                ...
                <div className="flex items-center gap-2 min-w-0">
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase bg-secondary text-secondary-foreground">{a.frequency}</span>
                  <span className="text-xs text-fg/50 truncate">
                    {a.destination || "email"}{a.filters?.location ? ` · ${String(a.filters.location)}` : ""}
                  </span>
                </div>
                <button onClick={() => handleDelete(a.id)} className="text-[11px] text-fg/40 hover:text-destructive transition-colors flex-shrink-0">Remove</button>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
